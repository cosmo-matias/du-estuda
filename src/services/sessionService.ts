...
export async function getSessionsByPlan(userId: string, planId: string): Promise<StudySession[]> {
  const q = query(sessionsRef, where("userId", "==", userId), where("planId", "==", planId));
  const snapshot = await getDocs(q);
